    ...
    def _update_network(self):
        ...
        with torch.no_grad():
            ...
            if deterministic_policy: 
                ...
                q_next_value, tdm_next_value = self.critic_target_network(inputs_next_norm_tensor, map_t(t), actions_next, return_sr=True) 
            else:  
            ...
            done = torch.tensor(transitions['t=0'])
            # target_tdm_value =  ((1-done)*(10*torch.tensor(transitions['diff']) + self.args.gamma*tdm_next_value.detach()) + 
            #                     done*10*torch.tensor(transitions['g']-transitions['ag_next']))
            c=1/self.env.distance_threshold
            ag = torch.tensor(self.g_norm.normalize(transitions['ag'])*gscale)
            ag_next = torch.tensor(self.g_norm.normalize(transitions['ag_next'])*gscale)
            diff = ag_next - ag
            final_dist = torch.tensor(g_norm)-ag
            scale = 0.1#((1-done)*diff + done*final_dist).std(axis=0).detach() + 1e-6
            diff /= scale
            final_dist /= scale
            target_tdm_value =  ((1-done)*(diff + tdm_next_value.detach()) + 
                                done*(final_dist))
            # target_q_value /= target_tdm_value.norm().detach()
            # clip the q value
            clip_return = 1 / (1 - self.args.gamma)
            target_q_value = torch.clamp(target_q_value, -clip_return*1000, 0)

        q0, tdm0 = self.critic_network(inputs_norm_tensor, map_t(t), actions_tensor, return_sr=True)                
        # critic_loss = 0*(target_q_value - q0).pow(2).mean() + (target_tdm_value - tdm0).pow(2).mean()                
        critic_loss = (target_q_value - q0).pow(2).mean() + (torch.tensor(g_next_norm) - tdm0).pow(2).mean()     
        # ipdb.set_trace()         
        # critic_loss = 0.00*((target_tdm_value**2).sum(axis=-1)**.5 - q0).pow(2).mean() + (target_tdm_value - tdm0).pow(2).mean()#/target_tdm_value.norm().detach() 



        # target_val = val(target_q_value, target_sr_value)
        # critic_loss = (target_val - val(q0, sr0)).pow(2).mean() + (target_sr_value - sr0).pow(2).mean()/50                            
        ...
        if self.global_count % 2 == 0:
            ...
            q, tdm = self.critic_network(duplicated_g_input, map_t(t), actions_real, return_sr=True)
            dist = (tdm**2).sum()#**.5
            dist = dist#/dist.detach()
            # actor_loss = dist + self.args.entropy_regularization*log_prob.mean()
            actor_loss = -q.mean() + self.args.entropy_regularization*log_prob.mean()
