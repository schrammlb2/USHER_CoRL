...
class her_sampler:
    def __init__(self, replay_strategy, replay_k, reward_func, gamma, two_goal, distance_threshold=0.05, 
        sr_dim=100, goal_dim=3, rff_function = None):
        ...
        self.rff = rff_function


    def kernel(self, x, y):
        return np.e**(-(((x-y)/self.distance_threshold)**2/2).sum(axis=-1))-1
    ...
    def sample_her_transitions(self, episode_batch, batch_size_in_transitions):
        ...
        try: 
            transitions['rff'] = self.rff(transitions['g'])
            transitions['diff'] = transitions['ag_next'] - transitions['ag']
        except: 
            pass
        # to get the params to re-compute reward
        # transitions['r'] = np.expand_dims(self.reward_func(transitions['ag_next'], transitions['g'], None), 1)
        # transitions['alt_r'] = np.expand_dims(self.reward_func(transitions['ag_next'], transitions['alt_g'], None), 1)
        transitions['r'] = np.expand_dims(self.kernel(transitions['ag_next'], transitions['g']), 1)
        transitions['alt_r'] = np.expand_dims(self.kernel(transitions['ag_next'], transitions['alt_g']), 1)
        # transitions['r'] = np.expand_dims((self.rff(transitions['ag_next'])*self.rff(transitions['g'])).sum(axis=-1), 1)
        # transitions['alt_r'] = np.expand_dims((self.rff(transitions['ag_next'])*self.rff(transitions['alt_g'])).sum(axis=-1), 1)
        ...
        transitions['t=0'] = np.expand_dims(transitions['t_remaining'] == 0, 1)*1
